...
class ProductController extends AbstractController
{
    ...
    public function frontaddToBasket(FlashyNotifier $flashy,Product $produit, SessionInterface $session): Response
{
    $panier = $session->get('panier', []);
    ...
    } elseif ($panier[$produit->getId()]['quantite'] < 1) {
        $panier[$produit->getId()]['quantite']++;
    } else {
        // Optional: add a flash message to indicate that the maximum quantity has been reached.
        $flashy->warning('Vous avez atteint la quantité maximale disponible pour ce produit.','http://localhost:8000/');
    }
...
// redirect back to the service page
    ...
    public function reclamerProduit(FlashyNotifier $flashy,HubInterface $hub,$description,Product $produit, Request $request,ReclamationRepository $reclamationRepository): RedirectResponse
    {
        ...
        $update = new Update(
            ...
            json_encode(['message' => '<div class="alert alert-success"> <img src="'.$this->getUser()->getGravatarUrl().'" alt="" class="rounded-circle">reclamation effectué par '.$this->getUser()->getNom().' Sur le produit:'.$produit->getNomProduit().'</div>'])
        );
        $hub->publish($update);
